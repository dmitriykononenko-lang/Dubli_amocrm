...
- Авто-слияние по правилам: при индексации (вебхук) однозначный дубль под правилом с
  `auto_merge` объединяется автоматически (режим `auto`, главная — более старая запись).
...
Правило с `auto_merge=true` включает **авто-слияние**: когда вебхук индексирует сущность
и она образует **однозначный** дубль (ровно один кандидат под таким правилом), пара
объединяется автоматически (главная — с меньшим `amo_id`), запись в журнале — `mode=auto`.
Неоднозначные случаи (0 или >1 кандидатов) остаются ручному режиму.
