...
export class SubscriptionsService {
  ...
  /** Состояние доступа: разрешён, если не приостановлен и now ≤ paid_till или now ≤ trial_ends_at. */
  async access(accountId: string): Promise<AccessState> {
    ...
    const dateOk = (paidTill != null && now <= paidTill) || (trialEndsAt != null && now <= trialEndsAt);
    const allowed = sub.status !== 'canceled' && dateOk;
    const horizon = paidTill && (!trialEndsAt || paidTill > trialEndsAt) ? paidTill : trialEndsAt;
  ...
  async recordPayment(input: {
    ...
    await this.repo.update(input.accountId, {
      status: 'active',
