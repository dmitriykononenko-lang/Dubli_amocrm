...
## Бэкапы (обязательно — диск эфемерный)
Локальный диск сервера **эфемерный** (данные пропадают при удалении/сбое, автобэкапов
нет). Поэтому настройте регулярные логические дампы с выгрузкой в S3:
```bash
# awscli для выгрузки в S3-совместимое хранилище (Selectel S3)
apt-get install -y awscli        # или: pip install awscli
# заполнить в .env: S3_BUCKET, S3_ENDPOINT, AWS_ACCESS_KEY_ID/SECRET_ACCESS_KEY
bash deploy/pg-backup.sh          # разовый дамп + выгрузка
crontab -e                        # добавить строку из deploy/pg-backup.cron (ежедневно 03:00)
```
Восстановление:
```bash
gunzip -c dubli-YYYYMMDD-HHMMSS.sql.gz | docker compose exec -T db psql -U dubli -d dubli
```

## Прод-эксплуатация
- **Бэкапы PostgreSQL** — см. раздел выше (дампы в S3; на эфемерном диске это критично).
